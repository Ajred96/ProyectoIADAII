...
\end{itemize}
    ...
    \item \textbf{Selección de la estrategia óptima:}
     \begin{itemize}
        \item Una vez procesados todos los grupos, se analiza el último nivel de la tabla $dp$ (el cuál corresponde a $dp[n]$, allí se encuentran todas las estrategias posibles que llegaron hasta el final respetando el esfuerzo máximo $(R_{max})$.
        \item Cada una de estas estrategias cuenta con una cantidad de esfuerzo utilizado y un número total de personas que no fueron moderadas.
        ...
        \item Con los conflictos recuperados, se calcula un conflicto promedio a partir de la división
    \end{itemize}
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{n:}
  Representa el número total de grupos de agentes que hay dentro de la red.

  \item \textbf{R\_max:}
  Esfuerzo máximo permitido.

  \item \textbf{n\_i:}
  Grupos individuales que hay en la red.
  ...
  Se indica como 8 Bytes, debido al uso de flotantes (en Python está preconfigurado como \texttt{float64}).

  \item \textbf{$\mu$:}
  Rendimiento de la máquina en operaciones por segundo, el cual se establecio como $2.09 \times 10^5$  (ajustado según valores obtenidos del análisis).
...
La formula utilizada en este caso es la de la complejidad espacial * el tamaño de celda, el cual quedaria de la siguiente manera:
    \[
        n^2 \cdot R_{max} \cdot k \cdot 8 bytes
    \]
...
\[
\frac{n \cdot (k+1) \cdot (R_{max}+1)}{2.09 \times 10^5}
\]
...
\end{itemize}
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{resources/bytes-segundos.png}
    \end{figure}
    ...
    \begin{quote}
    Por ejemplo, con $n = 15$ y $R_{\text{max}} = 474$ el tiempo teórico estimado es $\approx 0{,}65\ \text{s}$; con $n = 25$ y $R_{\text{max}} = 6\,465$ sube a $\approx 6{,}2\ \text{s}$; y aun para $n = 100$ y $R_{\text{max}} = 121\,144$ el tiempo previsto sería $\approx 23{,}19\ \text{min}$.
    \end{quote}
    ...
    \begin{quote}
    Por ejemplo, para $n = 25$ y $R_{\text{max}} = 1\,283$ la tabla cabe en $\approx 55\ \text{MB}$; para $n = 50$ y $R_{\text{max}} = 7\,959$ ocupa $\approx 0{,}6\ \text{GB}$; y solo para $n = 100$ y $R_{\text{max}} = 47\,096$ llega a $\approx 32\ \text{GB}$.
    \end{quote}
