...
\begin{itemize}
  ...
  \item \textbf{n:} Cantidad de grupos de la red social. Esta en terminos de k y se representa de la manera $n=2^k$.
...
La formula utilizada en este caso es la de la complejidad espacial multiplicado por el tamaño de celda, el cual quedaria de la siguiente manera:
    \[
        n^2 \cdot R_{max} \cdot k \cdot 4 bytes
    \]
    
    \item \textbf{Cual es la formula del tiempo teorico?}
La formula utilizada en este caso es la de la complejidad temporal dividido entre el rendimiento de la maquina en segundos ($\mu$) , el cual quedaria de la siguiente manera:
...
\begin{table}[H]
    ...
    \hline
    \end{tabular}
    \caption{Resultados ordenados por $k$ y tamaño de entrada $n$.}
    \label{tab:resultados_escala}
    \end{table}
    ...
    Cuando el número de grupos y el límite $R_{max}$ no son excesivamente altos, el algoritmo de programación dinámica consigue una solución óptima en un tiempo razonable. Sin embargo, para valores muy grandes, la cantidad de estados crece de forma considerable, y el tiempo de ejecución puede llegar a ser elevado. Aun así, representa una mejora sustancial frente a enfoques como fuerza bruta.

    \begin{quote}
    Por ejemplo, con $n = 512$ y $R_{\text{max}} = 1\,000$ el tiempo teórico estimado es $\approx 1{,}02\ \text{s}$; con $n = 16\,384$ y $R_{\text{max}} = 10\,000$ asciende a $\approx 8{,}19\ \text{min}$; y para $n = 524\,288$ y $R_{\text{max}} = 50\,000$ el tiempo estimado llega a $\approx 1{,}21\ \text{días}$.
    \end{quote}
\begin{itemize}
    \item Para valores de \( k \) entre 4 y 9, el tiempo de resolución es bueno, ya que varía de fracciones de segundo a pocos segundos, sin superar el minuto. Para k = 14 y $R_{\text{max}}$ = 1000, el tiempo de respuesta sigue siendo bueno aunque asciende casi al minuto.
    \item Para \( k=14 \) y $R_{\text{max}}$ mayores, el tiempo de respuesta se vuelve aceptable, siendo de varios minutos sin superar la hora. Para k=19 y $R_{\text{max}}$ bajos, el tiempo de respuesta sigue siendo aceptable
    \item Para los casos de k=19 y $R_{\text{max}}$ medianos y grandes, el tiempo de respuesta se vuelve inaceptable. Los valores de k\>19 tienen tiempos de respuesta de varias horas, incluso días o meses lo cual los vuelve completamente inaceptables
    \end{itemize}
    
    \paragraph{Conclusión Hipótesis Espacial:}

    El uso de memoria adicional para almacenar la tabla $dp$ es el principal factor en el costo espacial del método. Sin embargo, en la práctica, la poda de estados ineficientes y la restricción basada en el esfuerzo máximo $(R_{max})$ disminuyen notablemente el consumo de memoria, haciéndolo viable incluso en redes de gran tamaño.


    \begin{quote}
    Por ejemplo, para $n = 512$ y $R_{\text{max}} = 10\,000$ la tabla ocupa $\approx 94{,}37\ \text{GB}$; con $n = 16\,384$ y $R_{\text{max}} = 50\,000$ llega a $\approx 751{,}62\ \text{TB}$; y para $n = 16\,777\,216$ con $R_{\text{max}} = 10\,000$ se requieren hasta $\approx 270{,}22\ \text{EB}$.
    \end{quote}

\begin{itemize}
    \item Para $k$ = 4 el espacio utilizado es bueno, sin superar unos cuantos MB
    \item Para $k$ = 9 y $R_{\text{max}}$ pequeños y medianos, el espacio utilizado es aceptable, sin superar las 100GB
    \item Para $k$ = 9 y $R_{\text{max}}$ grandes, el espacio utilizado ascende casi a la media TB, lo cual se vuelve inaceptable. Para  \( k > 9 \) el espacio utilizado es completamente inaceptable, yendo desde descenas de TB hasta miles de EB.
\end{itemize}

\subsection*{Impacto de $R_{max}$}
...
Aunque para valores altos de Rmax se disponga de mayor libertad para moderar a más agentes, lo que intuitivamente sugiere que el problema se vuelve menos complejo, se observa que para casos en los que se tiene el mismo k pero aumenta Rmax el rendimiento empeora. Esto debido a que un valor más alto de Rmax implica evaluar aún más estados factibles para encontrar la solución optima.
