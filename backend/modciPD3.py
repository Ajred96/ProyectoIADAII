...
def esfuerzo_necesario(op1, op2, rigidez, e):
    ...
    return int(np.ceil(abs(op1 - op2) * rigidez * e))
...
def resolver_modci(n, opiniones_1, opiniones_2, rigidez, R_max):
    ...
    # Caso base: Sin esfuerzo, el conflicto es el original
    conflicto_inicial = calcular_conflicto(n, opiniones_1, opiniones_2)
    for r in range(R_max + 1):
        DP[r][0] = conflicto_inicial  # Inicializar la primera columna con el conflicto inicial
    ...
    for i in range(1, num_grupos + 1):  # Recorrer los grupos de agentes
        for r in range(R_max + 1):  # Recorrer los valores de esfuerzo
            ...
            for e in range(n[i-1] + 1):  # Probar cambiar desde 0 hasta todos los agentes del grupo
                ...
                if costo <= r:
                    nuevo_n = n.copy()
                    nuevo_n[i-1] -= e  # Reducir el número de agentes en conflicto
                    nuevo_conflicto = calcular_conflicto(nuevo_n, opiniones_1, opiniones_2)
                    posible_valor = DP[r - costo][i-1] + (nuevo_conflicto - DP[r - costo][i-1])
    ...
    for i in range(num_grupos, 0, -1):
        ...
        if r < 0:
            break  # Evita valores negativos en el esfuerzo
